...
- PR #16 Run `30159199959`에서 prerequisite·Independence PASS 뒤 Desktop Rust Type만 실패했으나 Cargo 원문이 없는 진단 한계를 그대로 보존했다.
- npm-ci 뒤·공통 Gate 앞에 정확히 같은 `npm run verify:desktop-type` 진단 Step을 추가했고 `continue-on-error` 없이 Fail-fast한다.
- 실패 시 Cargo stdout/stderr는 GitHub Job Log에 직접 남고, Fallback Evidence에는 고정 Allowlist의 정규화 Outcome만 기록한다. 원문 Cargo Output Artifact 저장은 추가하지 않았다.
- 공통 Gate 명령·정책·필수 범주·비밀정보 제외 계약과 제품 코드·설정·Dependency·Lockfile은 변경하지 않았다.
- 전용 Test RED `28/36`에서 GREEN `36/36`, 전체 순차 `218/218`, Independence `0`을 확인했다.
- GitHub PR 재실행은 어울1 소유이므로 수행하거나 원인 해결로 추론하지 않았다.
