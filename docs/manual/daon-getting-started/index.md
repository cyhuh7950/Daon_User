...
> 현재 Release는 `로그인 → Notebook 홈 → 새 Notebook 또는 기존 Notebook 선택 → 3열 작업 화면`을 연결합니다. Source·질문·Studio는 선택한 Notebook 범위에서만 동작하며, 외부 LLM 호출은 조직의 외부전송 정책이 허용한 Provider에만 전송됩니다.
...
1. 로그인하면 Notebook 홈이 열립니다.
2. `새 Notebook`을 눌러 제목과 선택 설명을 입력하거나 기존 Notebook 카드를 선택합니다.
3. 제품은 임의 Notebook을 자동 선택하지 않습니다. 사용자가 명시적으로 선택한 Notebook만 3열 작업 화면으로 엽니다.
4. 검색·최근 수정/제목 정렬·Grid/List 보기로 Notebook을 찾을 수 있습니다.
...
입력 조건: 근거 질문은 선택 Context가 필요합니다. `안녕하세요!` 같은 좁은 일반 대화는 Source 없이 실행할 수 있으며 `일반 대화 · 근거 미사용`으로 표시됩니다. LLM은 연결된 Provider의 사용 가능한 Model 중 사용자가 선택한 Model을 사용하며 자동으로 다른 Provider로 바꾸지 않습니다.
...
- Notebook 홈에서 생성한 Notebook은 즉시 빈 3열 작업 화면으로 열리고, 기존 Notebook은 저장된 선택 Context를 다시 검증한 뒤 엽니다.
...
- `EGRESS_POLICY_DENIED`: 조직 관리자가 허용 Provider·목적지·분류·마스킹 조건을 검토합니다. 사용자는 우회 주소나 다른 Provider로 자동 전환하지 않습니다.
