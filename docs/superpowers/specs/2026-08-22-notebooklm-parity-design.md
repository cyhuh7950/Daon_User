...
- MCP 서버: 사용자가 서버를 등록하고 연결을 유지하며 Notebook에서 선택해 사용
- Daon 승인 지식: Daon 승인 지식 Connector를 같은 연결 방식으로 등록하고 사용
...
- MCP·Daon 승인 지식 연결을 삭제하면 연결만 해제하며 외부 서버의 원본 데이터는 삭제하지 않는다.
...
두 연결형 Source는 동일한 등록·상태·선택 인터페이스를 사용한다.
