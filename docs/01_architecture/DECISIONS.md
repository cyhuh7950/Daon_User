...
| R1-D029 | 확정 | Egress 정책 정본은 불변 `EgressPolicyVersion`과 Organization/Workspace Binding으로 분리한다. Organization `deny_external`이 Workspace allow보다 우선하며 기존 Workspace는 명시적 deny로 Backfill한다. Run별 `EgressDecision`은 평가 결과로만 사용한다. Release 1에 정책 API/UI와 `organization_security_or_connector_policy_change` Step-up을 포함한다. `allow_approved_external` Question 전송은 현재 Membership Role이 `required_approver` 이상이고, exact actor·`external_transfer`·Run·effective policy fingerprint에 결속된 단기 Step-up을 1회 consume한 뒤에만 허용한다. 이를 위해 same-origin Question authorization preflight가 동일 Idempotency-Key와 실제 prepared wire payload를 검증한 opaque authorization을 발급하며 password는 Question POST·저장·로그에 포함하지 않는다. Question routing은 Canon provider kind와 `route_single_model()` 결과를 Decision에 고정한다. | 신산님 승인 · `APR-R1-M8-09-EGRESS-POLICY-C01-20260813-01` · 어울1 기술 판단 2026-08-13 |
