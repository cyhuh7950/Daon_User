...
| --- | --- | --- | --- |
| R1-D001 | 승인 요청 | Windows 11 23H2 이상, Chrome·Edge 최신 2개 주 버전, Android 12 이상, iOS 17 이상을 R1 최소 지원선으로 권고한다. | 신산님 G0 승인 필요 |
...
| R1-D003 | 승인 요청 | R1 Pilot은 Hybrid로 권고한다. 로컬 개발과 Windows Local-private, WSL 통합 환경, OCI Seoul Managed Cloud 운영 경로를 포함한다. On-prem 정식 배포는 R1 Pilot 범위에서 제외한다. | 신산님 G0 승인 필요 |
...
| R1-D009 | 승인 요청 | OCI Seoul을 기본 Region으로 권고한다. 원본·산출물 기본 보존은 Workspace 정책, 삭제 유예 30일, Audit 1년, RPO 15분, RTO 4시간을 Pilot 기본값으로 권고한다. Legal Hold가 있으면 삭제보다 우선한다. | 신산님 G0 승인 필요 |
| R1-D010 | 승인 요청 | Pilot 기본 한도는 파일당 100MB, Workspace당 20GB, 사용자 동시 Run 2개, 조직 동시 Run 20개로 권고한다. 상호작용 API p95 3초 이내(비동기 작업 제외), 비용 한도는 조직 관리자가 설정하고 초과 시 설계의 `COST_LIMIT_EXCEEDED`를 적용한다. | 신산님 G0 승인 필요; 실제 Pilot 규모로 조정 가능 |
...
- R1-D001·D003·D009·D010은 신산님의 제품·운영 기준 승인 대상이다.
- R1-D004·D006~D008·D011·D012는 외부 차단으로 분류되어 미정 상태를 숨기지 않는다. 관련 Work Order는 자격·장치·계약 확보 전 `BLOCKED`다.
- G0-BASELINE 승인은 위 분류와 조건부 진행을 승인하는 것이며 외부 차단 항목을 완료로 간주하지 않는다.
