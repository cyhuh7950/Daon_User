...
- 루트 `package.json`, 등록 구성요소의 모든 `package.json`·`pyproject.toml`, 루트 `package-lock.json`, `repo-boundaries.json`은 일반 문자열 제외 여부와 무관하게 구조 검사 계약에 포함된다. Lockfile은 외부 Registry URL·Integrity와 등록 Workspace의 `link/resolved` 메타데이터를 허용하되, 다른 Daon 제품 Package identity·의존 Spec 또는 승인되지 않은 로컬 경로는 허용하지 않는다. 승인 기준 Lockfile이 없거나 JSON/`packages` 구조를 읽을 수 없으면 검사 불능 Exit 2다.
...
Source Import는 JavaScript/TypeScript의 정적·동적 Import, `require()`, Side-effect Import와 `export ... from` 재수출을 포함한다. Python은 문장 위치의 `import module`, `import module as alias`, `from module import name`을 검사하며 주석·일반 문자열과 정상 외부 Package Import를 Import 위반으로 오인하지 않는다.
...
- Exit 2: Policy Schema 오류, 필수 Lockfile 누락·손상 또는 검사 불능.
