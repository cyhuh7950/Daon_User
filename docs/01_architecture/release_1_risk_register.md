...
| RSK-013 | 공유 ysna-server에서 다른 Project 자원과 충돌 | 중간/높음 | 기존 Container·Volume·Port 변화, Resource 경합 | `/home/ubuntu/deploy/daon-user` 전용 Compose Project·Network·Volume, 기존 `shared-db/common/netdata/proxy` 변경 금지, 배포 전후 Inventory Diff | 어울1·어울2 | Mitigating |
| RSK-014 | DB Migration 실패·Rollback 불능 | 중간/매우 높음 | Schema Drift, 적용 중단, Rollback 검증 부재 | 전용 PostgreSQL 18.4, 사전점검·Backup·Apply·Rollback Drill과 Migration Version 증거를 PR Merge 선행조건으로 적용 | 어울2·테스터 | Open |
| RSK-015 | ysna-server ARM64와 개발/운영 Image Architecture 불일치 | 중간/높음 | Image Pull/Native Module Build 실패 | ARM64 또는 Multi-arch Manifest, Native Dependency Build·Server Smoke Test, CI Artifact Architecture 기록 | 어울2 | Open |
