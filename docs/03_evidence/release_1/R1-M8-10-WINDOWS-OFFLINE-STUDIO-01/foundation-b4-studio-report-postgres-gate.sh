...
readonly DB="daon_b4_studio_report_it_20260815"
...
[[ "$CURRENT" == "0016" ]]
echo "B4_CURRENT_0016_PASS"
echo "B4_VERSION_CITATION_REVIEW_APPROVAL_EXPORT_PASS"

cleanup
if docker exec "$PG_CONTAINER" psql -U postgres -d postgres -lqt | grep -q "$DB"; then
  echo "B4_CLEANUP_FAILED"
  exit 1
fi
echo "B4_CLEANUP_REMAINING_0"
trap - EXIT
