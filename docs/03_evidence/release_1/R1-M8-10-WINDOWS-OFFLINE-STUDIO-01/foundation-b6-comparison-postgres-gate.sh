...
readonly DB="daon_b6_comparison_it_20260815"
...
[[ "$CURRENT" == "0016" ]]
echo "B6_CURRENT_0016_PASS"
echo "B6_COMPARISON_CONTENT_LINEAGE_VERSION_XLSX_PASS"

cleanup
if docker exec "$PG_CONTAINER" psql -U postgres -d postgres -lqt | grep -q "$DB"; then
  echo "B6_CLEANUP_FAILED"
  exit 1
fi
echo "B6_CLEANUP_REMAINING_0"
trap - EXIT
