#!/usr/bin/env bash
set -euo pipefail
readonly DB="daon_b8_business_draft_it_20260815"
...
trap cleanup EXIT
cleanup; docker exec "$PG_CONTAINER" createdb -U postgres -T template0 "$DB"
...
PYTHONPATH=src:tests "$UV" run --isolated --with pytest==9.0.3 --with "psycopg[binary]==3.3.4" --with psycopg-pool --with minio python -m pytest tests/test_studio_workspace_postgres.py::StudioWorkspaceRealPostgresIntegrationTests::test_rls_fk_transaction_and_registration_order -q
[[ "$(docker exec "$PG_CONTAINER" psql -U postgres -d "$DB" -Atc "select version_num from alembic_version")" == "0016" ]]
echo "B8_CURRENT_0016_PASS"
echo "B8_BUSINESS_DRAFT_CONTENT_LINEAGE_VERSION_DOCX_PASS"
cleanup
if docker exec "$PG_CONTAINER" psql -U postgres -d postgres -lqt | grep -q "$DB"; then exit 1; fi
echo "B8_CLEANUP_REMAINING_0"
trap - EXIT
