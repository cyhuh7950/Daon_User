#!/usr/bin/env bash
set -euo pipefail
readonly DB="daon_b7_knowledge_map_it_20260815"
...
[[ "$CURRENT" == "0016" ]]
echo "B7_CURRENT_0016_PASS"
echo "B7_KNOWLEDGE_MAP_CONTENT_LINEAGE_VERSION_JSON_PASS"
cleanup
if docker exec "$PG_CONTAINER" psql -U postgres -d postgres -lqt | grep -q "$DB"; then exit 1; fi
echo "B7_CLEANUP_REMAINING_0"
trap - EXIT
