...
permission_stage_failure_code_from_log() {
  local log_file="$1"
  local marker=""
  marker="$(grep -Eo 'DAON_PERMISSION_XCTEST_STAGE=(PHASE_EXPECTED_BINDING|APP_LAUNCH_ROOT|CAMERA_REQUEST|CAMERA_RESULT|MICROPHONE_REQUEST|MICROPHONE_RESULT|NOTIFICATION_REQUEST|ALERT_TITLE|ALERT_COUNT|ALERT_ALLOW|ALERT_DISMISSAL|SETTINGS_FOREGROUND|SETTINGS_NOTIFICATION_ROW|SETTINGS_SWITCH_READ|SETTINGS_SWITCH_TOGGLE|SETTINGS_SWITCH_VERIFY|APP_RETURN_ROOT|NOTIFICATION_RESULT)$' "${log_file}" | tail -n 1 || true)"
  if [[ -n "${marker}" ]]; then
