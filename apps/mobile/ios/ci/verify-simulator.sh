...
is_allowed_sim_stage() {
  case "$1" in
    INITIALIZE|APP_ARTIFACT|BOOT_STATUS|INSTALL|INITIAL_TERMINATE|ROUTE_CLEAR|LAUNCH|HOME_READY|PERMISSION_GRANT_INITIAL|PERMISSION_REVOKE|PERMISSION_GRANT_AGAIN|LIFECYCLE_PREPARE_LAUNCH|LIFECYCLE_PREPARE_READY|LIFECYCLE_APPEARANCE|LIFECYCLE_TERMINATE|LIFECYCLE_RELAUNCH|LIFECYCLE_READY|LIFECYCLE_STATE|FINAL_LOG_CAPTURE|FINAL_LOG_SCAN|FINAL_BINARY_SCAN|FINAL_TERMINATE|FINAL_PROCESS_CHECK|STATUS_WRITE|UNCLASSIFIED) return 0 ;;
    *) return 1 ;;
  esac
}
...
DAON_SIM_STAGE="LIFECYCLE_PREPARE_LAUNCH"
xcrun simctl launch "${SIMULATOR_UDID}" "${BUNDLE_ID}" | tee "${EVIDENCE_DIR}/lifecycle-prepare-launch.log"
DAON_SIM_STAGE="LIFECYCLE_PREPARE_READY"
wait_for_route_with_evidence Home
