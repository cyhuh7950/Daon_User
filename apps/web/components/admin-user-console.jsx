...
const rolesFor = (user) => user.protected ? ["관리자", "전문가"] : ["전문가"];
...
export function AdminUserConsole({
  ...
  const openCreate = () => {
    setForm({ login_id: "", email: "", initial_password: "", state: "active", roles: ["전문가"] });
  ...
  return <main className="admin-user-page">
    ...
    {state === "ready" && <section className="admin-user-card" aria-labelledby="admin-user-list-title">
      ...
      {visibleUsers.length === 0 ? <p className="admin-user-empty">조건에 맞는 사용자가 없습니다.</p> : <div className="admin-user-table-wrap"><table className="admin-user-table"><thead><tr><th scope="col"><input type="checkbox" aria-label="전체 사용자 선택" checked={visibleUsers.some((user) => !user.protected) && visibleUsers.filter((user) => !user.protected).every((user) => selected.has(user.user_id))} onChange={(event) => toggleAll(event.target.checked)} /></th><th scope="col">사용자</th><th scope="col">이메일</th><th scope="col">상태</th><th scope="col">역할</th><th scope="col">관리</th></tr></thead><tbody>{visibleUsers.map((user) => <tr key={user.user_id}><td><input type="checkbox" aria-label={`${user.login_id ?? user.user_id} 선택`} disabled={user.protected} checked={selected.has(user.user_id)} onChange={(event) => toggleSelected(user.user_id, event.target.checked)} /></td><td><strong>{user.login_id ?? "로그인 ID 없음"}</strong><small>{user.user_id}</small>{user.protected && <small>보호된 시스템 관리자</small>}</td><td>{user.email ?? "이메일 없음"}</td><td><span className={`admin-user-status status-${user.state}`}>{stateLabel[user.state] ?? user.state}</span></td><td><div className="admin-user-role-list">{rolesFor(user).map((role) => <span key={role}>{role}</span>)}</div></td><td><div className="admin-user-row-actions"><button type="button" disabled={!canChangeState(user) || pending.has(user.user_id)} aria-label={`${user.login_id ?? user.user_id} 계정 상태 변경`} onClick={() => void changeState(user)}>{user.protected ? "보호됨" : pending.has(user.user_id) ? "처리 중…" : user.state === "active" ? "중지" : user.state === "suspended" ? "재활성화" : user.state === "pending_email" ? "인증 대기" : stateLabel[user.state]}</button>{canApprove(user) && <button type="button" onClick={() => void approve(user)}>승인</button>}{!user.protected && <><button type="button" onClick={() => openEdit(user)}>수정</button><button type="button" onClick={() => void reset(user)} disabled={!user.has_email}>비밀번호 초기화</button><button type="button" onClick={() => void remove(user)}>삭제</button></>}</div></td></tr>)}</tbody></table></div>}
    </section>}
    </div>
