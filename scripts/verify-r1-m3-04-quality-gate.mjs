...
try {
  ...
  policy.artifacts = {
    result: "docs/03_evidence/release_1/R1-M3-04/quality-gate-result.json",
    summary: "docs/03_evidence/release_1/R1-M3-04/quality-gate-summary.md"
  };
  const result = await runQualityGate({ root, policy, policyPath });
  console.log(result.summary);
  process.exitCode = result.exitCode;
} catch (error) {
  console.error(`R1_M3_04_QUALITY_GATE_EXECUTION_ERROR ${error?.code ?? error?.name ?? "ERROR"}`);
  process.exitCode = 2;
}
