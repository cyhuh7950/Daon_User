...
import { StudioWorkflowPane } from "./studio-workflow-pane.jsx";
...
function Pane({ pane, state, workspaceId, onUploadPdf, actualQuestion, onOpenEvidence, onSetCursor, onSelectSource, onSourceKnowledgeAction, onRunModelAction, onStudioAction }) {
  if (pane === "knowledge") return <SourceKnowledgePane workspaceId={workspaceId} onUploadPdf={onUploadPdf} selectedSourceId={state.selected_source_id} domainState={state.source_knowledge} onDomainAction={onSourceKnowledgeAction} onSelectSource={onSelectSource} onOpenEvidence={onOpenEvidence} />;
  if (pane === "conversation") return <RunModelEvidencePane actualQuestion={actualQuestion} domainState={state.run_model} onDomainAction={onRunModelAction} onOpenEvidence={onOpenEvidence} />;
  return <StudioWorkflowPane domainState={state.studio_workflow} onStudioAction={onStudioAction} onOpenEvidence={onOpenEvidence} onSetCursor={onSetCursor} />;
}

export function AdaptiveWorkspace({ workspaceId = "", routeId = "workspace_detail", screenId = "workspace_detail", onUploadPdf, actualQuestion = null }) {
  ...
  return (
    <main className={`adaptive-workspace mode-${projection.layoutMode}`} data-workspace-id={workspaceId} data-layout-mode={projection.layoutMode} data-route-id={routeId} data-screen-id={screenId} data-accessibility-standard={accessibilityStandard} data-selected-source={viewState.selected_source_id} data-conversation-id={viewState.conversation_id} data-run-id={viewState.run_id} data-run-status={viewState.run_status} data-artifact-id={viewState.artifact_id} data-artifact-cursor={viewState.artifact_cursor} data-evidence-id={viewState.evidence_id} data-evidence-position={viewState.evidence_position} data-evidence-source={viewState.evidence_source_id} data-evidence-source-version={viewState.evidence_source_version_id}>
