...
if os.name == "nt":
    ...
    def _validate_path(root: Path, path: Path, *, require_single_link: bool) -> None:
        stat = path.lstat()
        if path.is_symlink() or getattr(stat, "st_file_attributes", 0) & _REPARSE_ATTRIBUTE:
            raise UnsafePathError("link")
        if require_single_link and stat.st_nlink != 1:
            raise UnsafePathError("hardlink")
        if not _contained(root.resolve(), path.resolve()):
            raise UnsafePathError("outside root")

    def validate_directory_chain(root: Path, directory: Path) -> None:
        current = directory
        while True:
            _validate_path(root, current, require_single_link=False)
            if current == root:
                return
    ...
    def read_file(root: Path, path: Path) -> bytes:
        ...
        with path.open("rb") as stream:
            _validate_path(root, path, require_single_link=True)
            return stream.read()
    ...
    def atomic_write(root: Path, directory: Path, temporary: Path, destination: Path, payload: bytes) -> None:
        ...
        try:
            with temporary.open("xb") as stream:
                _validate_path(root, temporary, require_single_link=True)
