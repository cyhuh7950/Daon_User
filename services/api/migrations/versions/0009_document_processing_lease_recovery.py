"""Reclaim and audit expired document-worker leases."""
...
def _claim_function(*, reclaim_expired: bool) -> str:
    candidate = (
        "(job.state IN ('pending','retry_wait') AND "
        "job.next_attempt_at <= clock_timestamp())"
    )
    lease_loss = ""
    dependency = ""
    if reclaim_expired:
        candidate += " OR (job.state = 'leased' AND job.lease_until <= clock_timestamp())"
        lease_loss = """
          , lease_loss AS (
            ...
            SELECT tenant_id,workspace_id,job_id,attempt,lease_owner,'lease_lost',
              'DOCUMENT_WORKER_LEASE_EXPIRED',trace_id,created_at,clock_timestamp()
            FROM candidate WHERE state='leased'
            ON CONFLICT (tenant_id,workspace_id,job_id,attempt_number) DO NOTHING
            RETURNING 1
          )
        """
        dependency = " AND (SELECT count(*) FROM lease_loss) >= 0"
    return f"""
        ...
        BEGIN
          ...
          WHERE job.tenant_id=candidate.tenant_id
            ...
            AND job.job_id=candidate.job_id{dependency}
...
def upgrade() -> None:
    op.execute(_claim_function(reclaim_expired=True))


def downgrade() -> None:
    op.execute(_claim_function(reclaim_expired=False))
